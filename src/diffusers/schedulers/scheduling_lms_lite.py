...
class LMSLiteScheduler(SchedulerMixin, ConfigMixin):
    """Contract-only LMS-style scheduler scaffold (no sampler math yet)."""
    ...
    def set_timesteps(self, num_inference_steps: int, device: Union[str, torch.device, None] = None):
        ...
        timesteps = torch.flip((torch.arange(0, num_inference_steps) * step).round(), dims=[0]).clone()
